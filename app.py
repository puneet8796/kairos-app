...
if "result" in st.session_state:
    ...
    # CHANGE 3: one-day disclaimer
    st.caption("This is one day. Run it tomorrow and it reads differently — because your days differ.")
    if p:
        risk_pill(p["risk"])
        st.write("")
        st.write(p["desc"])
    ...
    # ----------------------------------------------------------------------- Feedback
    # CHANGE 1: feedback section with always-visible "Send note" button for mobile.
    # The button is rendered unconditionally once this section is reached —
    # on Android there is no Ctrl+Enter to submit a text area, so the button
    # must not be gated behind fb_note having content.
    st.divider()
    st.markdown("#### How did this land?")
    fb_rating = st.radio(
        "Rating",
        options=["Nailed it", "Close", "Missed the mark"],
        horizontal=True,
        label_visibility="collapsed",
        key="fb_rating",
    )
    fb_note = st.text_area(
        ...
        placeholder="Anything that felt off or could be sharper...",
        height=80,
        key="fb_note",
    )
    # Always show the button — visible and tappable on mobile as soon as
    # the feedback section renders. Do not condition on fb_note content.
    if st.button("Send note", use_container_width=True, key="fb_send"):
        if fb_note.strip() or fb_rating:
            st.success("Thanks — noted.")
        else:
            st.info("Select a rating or add a note first.")
